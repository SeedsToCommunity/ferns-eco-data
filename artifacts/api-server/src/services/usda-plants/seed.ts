import { db, fernsSourcesTable } from "@workspace/db";
import { USDA_PLANTS_REGISTRY_ENTRY } from "./metadata.js";
import { logger } from "../../lib/logger.js";

let seeded = false;

export async function ensureUsdaPlantsRegistryEntry(): Promise<void> {
  if (seeded) return;
  try {
    await db
      .insert(fernsSourcesTable)
      .values(USDA_PLANTS_REGISTRY_ENTRY)
      .onConflictDoUpdate({
        target: fernsSourcesTable.source_id,
        set: {
          name: USDA_PLANTS_REGISTRY_ENTRY.name,
          status: USDA_PLANTS_REGISTRY_ENTRY.status,
          description: USDA_PLANTS_REGISTRY_ENTRY.description,
          input_summary: USDA_PLANTS_REGISTRY_ENTRY.input_summary,
          output_summary: USDA_PLANTS_REGISTRY_ENTRY.output_summary,
          update_frequency: USDA_PLANTS_REGISTRY_ENTRY.update_frequency,
          known_limitations: USDA_PLANTS_REGISTRY_ENTRY.known_limitations,
          metadata_url: USDA_PLANTS_REGISTRY_ENTRY.metadata_url,
          explorer_url: USDA_PLANTS_REGISTRY_ENTRY.explorer_url,
          permission_granted: USDA_PLANTS_REGISTRY_ENTRY.permission_granted ?? null,
          permission_status: USDA_PLANTS_REGISTRY_ENTRY.permission_status ?? null,
          general_summary: USDA_PLANTS_REGISTRY_ENTRY.general_summary ?? null,
          technical_details: USDA_PLANTS_REGISTRY_ENTRY.technical_details ?? null,
          updated_at: new Date(),
        },
      });
    seeded = true;
  } catch (err) {
    logger.error({ err }, "Failed to seed USDA PLANTS registry entry");
  }
}
