...
export const GetBonapMapResponse = zod.object({
  "source_url": zod.string().nullable().describe('Canonical BONAP page URL for this species or genus. Always constructed when possible and returned regardless of whether a map was found. Enables \'View on BONAP\' links in applications.\n'),
  "found": zod.boolean().describe('Whether a BONAP map was found for this query'),
  "data": zod.object({
  ...
  "color_key_url": zod.string().describe('URL of the BONAP map key reference page'),
  "color_key_image_url": zod.string().describe('Direct URL to BONAP\'s authoritative composite color key GIF image. Display this image to show users the complete, pixel-perfect color key.\n'),
  ...
  "data_vintage": zod.string().describe('NAPA maps: December 15, 2014. Color key documentation: February 8, 2024.'),
  "licenses": zod.array(zod.string()).describe('Whether FERNS has received written permission from BONAP to reproduce and display map materials. Currently false. Applications should display this when restricted licenses apply.\n'),
  "license_notes": zod.string().describe('Human-readable permission status string. Matches the value from the metadata endpoint. Applications should display license_notes when restricted licenses apply.\n'),
  ...
  "queried_at": zod.date(),
  "note": zod.string().nullish().describe('Human-readable diagnostic note. Non-null only when status is unverified — explains that BONAP\'s server did not return a definitive response during cache population and instructs the caller to retry with ?refresh=true. Null when status is found or not_found.\n')
}).nullish(),
...
export const GetGbifMatchResponse = zod.object({
  ...
  "found": zod.boolean(),
  "data": zod.object({
  "usageKey": zod.number().optional().describe('GBIF backbone usageKey. Absent when matchType is NONE.'),
  "canonicalName": zod.string().nullish(),
  "scientificName": zod.string().nullish(),
  "rank": zod.string().nullish(),
  "status": zod.string().nullish().describe('ACCEPTED, SYNONYM, DOUBTFUL, or other GBIF taxonomic status'),
  "acceptedUsageKey": zod.number().optional().describe('Present when status is SYNONYM (verbatim GBIF field)'),
...
}).describe('The verbatim GBIF backbone match response. All fields are passed through unchanged from the GBIF species\/match API. Key fields include usageKey, canonicalName, scientificName, rank, status, confidence, matchType, classification (kingdom through species with corresponding \*Key integers), and acceptedUsageKey when status is SYNONYM. Additional fields may be present as returned by GBIF.\n').nullable(),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetGbifOccurrencesQueryParams = zod.object({
  ...
  "countries": zod.string().optional().describe('Comma-separated ISO 3166-1 alpha-2 country codes (e.g. US,CA,MX). Mutually exclusive with continent and bbox.\n'),
  "continent": zod.enum(['AFRICA', 'ANTARCTICA', 'ASIA', 'EUROPE', 'NORTH_AMERICA', 'OCEANIA', 'SOUTH_AMERICA']).optional().describe('GBIF continent value. One of AFRICA, ANTARCTICA, ASIA, EUROPE, NORTH_AMERICA, OCEANIA, SOUTH_AMERICA. Mutually exclusive with countries and bbox.\n'),
  "bbox": zod.string().optional().describe('Bounding box as minLat,minLon,maxLat,maxLon (decimal degrees, WGS84). e.g. 24.396,-125.0,49.384,-66.93 for continental US. Mutually exclusive with countries and continent.\n'),
...
export const GetGbifOccurrencesResponse = zod.object({
  ...
  "geography_params": zod.string().describe('Serialized geography parameters for display'),
  "occurrence_count": zod.number(),
  "occurrence_count_us": zod.number().nullish(),
  "recent_occurrences": zod.array(zod.object({
  "gbifID": zod.string(),
  ...
  "source_url": zod.string().nullable().describe('GBIF occurrence search URL for this taxon and geography')
}).nullable(),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
...
export const GetGbifSpeciesSynonymsQueryParams = zod.object({
  ...
  "provenance_verbosity": zod.enum(['full', 'summary', 'none']).optional().describe('Controls provenance text fields: full | summary | none')
})
...
export const GetGbifSpeciesSynonymsResponse = zod.object({
  ...
  "found": zod.boolean(),
  "data": zod.object({
  "offset": zod.number().describe('Zero-based offset of the first result (mirrors upstream envelope)'),
  "limit": zod.number().describe('Maximum number of results requested (mirrors upstream envelope)'),
  "endOfRecords": zod.boolean().describe('True when no more results are available after this page (mirrors upstream envelope)'),
  "count": zod.number().describe('Total number of synonyms for this taxon (mirrors upstream envelope)'),
...
})).describe('Synonym records as returned by GBIF (verbatim upstream fields)')
}).nullable(),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
...
export const GetGbifSpeciesVernacularNamesResponse = zod.object({
  ...
  "limit": zod.number().describe('Maximum number of results requested (mirrors upstream envelope)'),
  "endOfRecords": zod.boolean().describe('True when no more results are available after this page (mirrors upstream envelope)'),
  "count": zod.number().describe('Total number of vernacular names for this taxon (mirrors upstream envelope)'),
  "results": zod.array(zod.object({
  "vernacularName": zod.string().optional(),
  "language": zod.string().optional(),
  "country": zod.string().nullish(),
  "source": zod.string().nullish()
})).describe('Vernacular name records as returned by GBIF (verbatim upstream fields)')
}).nullable(),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
...
}).optional().describe('Full registry entry for this GBIF service source'),
  "queried_at": zod.date(),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
...
export const GetInatPlacesAutocompleteResponse = zod.object({
  "source_url": zod.string().nullable().describe('The iNaturalist autocomplete API URL that was called to resolve this query. Null if unavailable.'),
...
}).describe('iNaturalist place record passed through verbatim from the iNaturalist autocomplete API.')).describe('Up to 5 matching places. Applications select the correct one.'),
...
export const GetInatObservationsHistogramResponse = zod.object({
  "source_url": zod.string().describe('https:\/\/www.inaturalist.org\/observations?taxon_id={id}&place_id={ids}'),
  "found": zod.boolean(),
  "cache_status": zod.enum(['hit', 'miss', 'bypassed']),
  "queried_at": zod.date().describe('When this FERNS request was processed'),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist observations\/histogram response. The month_of_year object inside results contains observation counts keyed by month number string (\'1\' through \'12\').\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatObservationsResponse = zod.object({
  ...
  "found": zod.boolean(),
  "data": zod.object({
  "total_results": zod.number().describe('Total observations matching the query (iNat API limit is 10,000)'),
  "page": zod.number(),
  "per_page": zod.number(),
  ...
  "uri": zod.string().describe('Canonical URL to the full observation on iNaturalist'),
  "observed_on": zod.string().nullish().describe('Date the observation was made (YYYY-MM-DD)'),
  "quality_grade": zod.string().nullish().describe('research | needs_id | casual'),
  "obscured": zod.boolean().nullish().describe('Whether the location has been obscured for this observation'),
  "license_code": zod.string().nullish().describe('License code for this observation (e.g. cc-by, cc-by-nc)'),
  "description": zod.string().nullish().describe('Observer-provided description text'),
  "tags": zod.array(zod.string()).nullish().describe('Tags applied to this observation'),
  "taxon_name": zod.string().nullish().describe('Scientific name (shorthand from taxon.name)'),
  "common_name": zod.string().nullish().describe('Preferred common name (shorthand from taxon.preferred_common_name)'),
  "taxon": zod.object({
  "id": zod.number().optional(),
  "rank": zod.string().nullish(),
  "iconic_taxon_name": zod.string().nullish(),
  "default_photo": zod.record(zod.string(), zod.unknown()).nullish()
}).nullish().describe('Taxon sub-object with key fields'),
  "place_guess": zod.string().nullish().describe('Human-readable location string provided by the observer'),
  "location": zod.string().nullish().describe('lat,lng decimal string'),
  "observer": zod.string().nullish().describe('iNaturalist login of the observer (shorthand from user.login)'),
  "user": zod.object({
  "id": zod.number().optional(),
  "name": zod.string().nullish(),
  "login": zod.string().nullish()
}).nullish().describe('Observer user sub-object'),
  "photo_url": zod.string().nullish().describe('Medium-size URL of the first photo (shorthand)'),
  "photo_attribution": zod.string().nullish().describe('License and attribution text for the first photo (shorthand)'),
  "photos": zod.array(zod.record(zod.string(), zod.unknown())).nullish().describe('Full photos array from iNaturalist'),
  "annotations": zod.array(zod.record(zod.string(), zod.unknown())).nullish().describe('Controlled annotation values on this observation'),
  "ofvs": zod.array(zod.record(zod.string(), zod.unknown())).nullish().describe('Observer field values (OFVs) on this observation')
}).describe('Expanded field subset of a single iNaturalist observation. Follow the uri field for the complete record on iNaturalist. Note: nativity data (native, introduced, endemic status) is NOT present on observation records — it lives in listed_taxa on the taxon record, available via GET \/inat\/taxa\/{id}.\n'))
}),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
...
export const GetInatObservationsSpeciesCountsResponse = zod.object({
  "source_url": zod.string().describe('https:\/\/www.inaturalist.org\/observations?taxon_id=...'),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist observations\/species_counts response. The results array contains entries with count and taxon fields. Each taxon includes id, name, preferred_common_name, default_photo, and observations_count.\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatControlledTermsResponse = zod.object({
  ...
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist controlled_terms response.'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatControlledTermsForTaxonResponse = zod.object({
  ...
  "found": zod.boolean(),
  "cache_status": zod.string().optional(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist controlled_terms\/for_taxon response.'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatTaxaAutocompleteResponse = zod.object({
  "source_url": zod.string(),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist taxa\/autocomplete response with results array.'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatTaxaByIdResponse = zod.object({
  ...
  "found": zod.boolean(),
  "cache_status": zod.string().optional(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist taxa\/{id} response. Includes wikipedia_summary, listed_taxa (nativity per place), children, conservation_status, and default_photo. The listed_taxa array is the canonical source of native\/introduced\/endemic status for this taxon.\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatPlacesByIdResponse = zod.object({
  ...
  "found": zod.boolean(),
  "cache_status": zod.string().optional(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist places\/{id} response. Includes display_name, place_type, centroid, bounding_box_geojson, and admin hierarchy (admin_level, parent_id).\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatPlacesNearbyResponse = zod.object({
  "source_url": zod.string(),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist places\/nearby response. Contains two result arrays: standard (admin places) and community (user-created places).\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatObservationsTaxonSummaryResponse = zod.object({
  "source_url": zod.string(),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist taxon_summary response for the given observation. Contains wikipedia_summary (string), listed_taxon (establishment_means for the observation\'s place), and conservation_status.\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatIdentificationsSimilarSpeciesResponse = zod.object({
  "source_url": zod.string(),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist identifications\/similar_species response. The results array contains taxa frequently confused with the queried taxon, each with a count field indicating co-confusion frequency.\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatIdentificationsSpeciesCountsResponse = zod.object({
  "source_url": zod.string(),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist identifications\/species_counts response. Species ranked by identification activity — a community engagement signal distinct from raw observation count. Supports taxon_of=identification|community.\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatIdentificationsRecentTaxaResponse = zod.object({
  "source_url": zod.string(),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist identifications\/recent_taxa response. Taxa recently identified in a place — \"what\'s being documented right now\".\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatIdentificationsResponse = zod.object({
  "source_url": zod.string(),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist identifications response. Contains total_results, page, per_page, and a results array of individual identification records. Each record includes id, created_at, taxon, observation, and user fields.\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatIdentificationsByIdResponse = zod.object({
  "source_url": zod.string(),
  "found": zod.boolean(),
  "data": zod.record(zod.string(), zod.unknown()).describe('Raw iNaturalist identification record response for a single identification ID. Contains the full identification record including taxon, observation, user, and created_at fields.\n'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
  "method": zod.string().describe('How the data was obtained: api_fetch | blob_import | llm_synthesis'),
...
export const GetInatMetadataResponse = zod.object({
  "service_id": zod.string(),
  "service_name": zod.string(),
  "licenses": zod.array(zod.string()).describe('True if FERNS has permission to use and expose this data. iNaturalist is open access — no permission request required.'),
  "license_notes": zod.string(),
  "attribution": zod.object({
  "source_name": zod.string(),
  "website": zod.string(),
  ...
  "metadata_url": zod.string().optional(),
  "explorer_url": zod.string().optional()
}).optional().describe('Full registry entry for this iNaturalist service source'),
  "queried_at": zod.date(),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
...
export const GetSourcesIndexResponse = zod.object({
  ...
  "explorer_url": zod.string().describe('Link to this service\'s Source Explorer UI'),
...
export const GetMnfiMetadataResponse = zod.object({
  ...
  "service_name": zod.string(),
  "found": zod.boolean(),
  "community_count": zod.number().optional().describe('Total number of natural community types loaded into FERNS'),
  "county_element_count": zod.number().optional().describe('Total number of county element occurrence records loaded'),
  "plant_count": zod.number().optional().describe('Total number of characteristic plant records loaded'),
  "class_summary": zod.array(zod.object({
  "community_class": zod.string().optional(),
  "count": zod.number().optional()
})).optional().describe('Community counts grouped by community class'),
  "counties_with_data": zod.array(zod.string()).optional().describe('List of Michigan counties with at least one element occurrence'),
...
export const GetMnfiCommunityResponse = zod.object({
  ...
  "characteristic_plants": zod.object({
  "total_entries": zod.number().optional(),
  "plant_list_url": zod.string().optional(),
  "by_life_form": zod.record(zod.string(), zod.array(zod.object({
  "common_name": zod.string().optional(),
  "scientific_names": zod.array(zod.string()).optional()
}))).optional()
}).optional()
}).nullish().describe('Community record with characteristic plants, or null if not found')
})
...
export const GetMnfiCommunityPlantsResponse = zod.object({
  ...
  "common_name": zod.string().optional(),
  "scientific_names": zod.array(zod.string()).optional()
}))).optional().describe('Plants grouped by life form (tree, shrub, herb, graminoid, vine, etc.)')
}).optional()
})
...
export const GetNatureserveSpeciesResponse = zod.object({
  ...
  "state_status_note": zod.string().nullish().describe('Clarifying note when state_status is present: the value is derived from the NatureServe S-rank and reflects rarity status, not a formal statutory state listing.\n'),
...
export const GetNatureserveSearchResponse = zod.object({
  ...
  "result_count": zod.number().optional().describe('Number of records in this page'),
  "total_results": zod.number().optional().describe('Total matching records across all pages'),
...
export const GetUsdaPlantsResponse = zod.object({
  ...
  "data": zod.object({
  "species": zod.string().optional().describe('The queried species name (FERNS input echo)'),
  "Symbol": zod.string().nullish().describe('USDA PLANTS symbol code'),
  "ScientificName": zod.string().nullish().describe('Scientific name as returned by USDA PLANTS'),
  "CommonName": zod.string().nullish().describe('Common name as returned by USDA PLANTS'),
  "Rank": zod.string().nullish().describe('Taxonomic rank as returned by USDA PLANTS'),
  "usda_id": zod.number().nullish().describe('USDA PLANTS internal numeric ID (FERNS derived)'),
  "profile_url": zod.string().nullish().describe('URL to USDA PLANTS profile page (FERNS derived)'),
  "NativeStatuses": zod.array(zod.object({
  ...
  "WetlandData": zod.array(zod.record(zod.string(), zod.unknown())).nullish(),
  "LegalStatuses": zod.array(zod.record(zod.string(), zod.unknown())).nullish(),
  "Durations": zod.array(zod.string()).nullish(),
  "GrowthHabits": zod.array(zod.string()).nullish(),
  "Group": zod.string().nullish(),
  ...
  "FactSheetUrls": zod.array(zod.string()).optional(),
  "PlantGuideUrls": zod.array(zod.string()).optional(),
  "OtherCommonNames": zod.array(zod.string()).nullish(),
  "ProfileImageFilename": zod.string().nullish()
}).describe('USDA PLANTS species data. PascalCase fields (Symbol, ScientificName, CommonName, Rank, NativeStatuses, etc.) are passed through verbatim from USDA PLANTS. snake_case fields (species, usda_id, profile_url) are FERNS-derived.\n')
})
...
export const GetLadyBirdJohnsonResponse = zod.object({
  "found": zod.boolean().describe('True when HTTP 200 returned for the profile URL (status=\"found\").'),
  "queried_at": zod.date(),
  "source_url": zod.string().optional(),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  ...
  "found": zod.boolean().optional().describe('True when status is \"found\".'),
  ...
  "cache_hit": zod.boolean().optional().describe('True if the result was served from lbj_url_cache.')
}).optional()
}).describe('Response from \/lady-bird-johnson — profile URL verification result keyed by USDA Plants symbol.\n')
...
export const GetLadyBirdJohnsonSpeciesTextResponse = zod.object({
  ...
  "queried_at": zod.date(),
  "source_url": zod.string().optional(),
  "cache_status": zod.enum(['hit', 'miss']).describe('\"hit\" — returned from species_page_text_cache. \"miss\" — live fetch performed.\n'),
  "fetch_error": zod.string().optional().describe('Present only when a transient upstream error prevented the scrape. Not cached.\n'),
  "scraped_at": zod.date().optional().describe('Timestamp of when the text was originally scraped.'),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
  "fetched_at": zod.date().describe('When this record was obtained from the source'),
...
}).describe('Response from \/lady-bird-johnson\/species-text — scraped species page text keyed by USDA Plants symbol.\n')
...
export const GetAnnArborNpnSpeciesBulkResponse = zod.object({
  "found": zod.boolean().describe('false when the table is empty (import not yet run)'),
  "queried_at": zod.date(),
  "source_url": zod.string(),
  "provenance": zod.object({
  "source_id": zod.string().describe('Stable identifier for this data source (e.g. bonap-napa)'),
...
export const GetAnnArborNpnSpeciesByKeyResponse = zod.object({
  ...
  "source_url": zod.string(),
  "scraped_at": zod.date()
}).nullish().describe('A single species record from The Native Plant Nursery database.')
...
export const GetAnnArborNpnNamesResponse = zod.object({
  "found": zod.boolean(),
  "queried_at": zod.date(),
  "source_url": zod.string(),
  "provenance": zod.object({
...
export const ImportAnnArborNpnBody = zod.object({
  "acronyms": zod.array(zod.string()).optional().describe('Optional subset of acronyms to import; omit to import all')
})
