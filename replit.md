...
3.  **Knowledge API**: Provides programmatic access to the data. Every EC route path after the source identifier prefix MUST use the upstream source's own path verbatim — this is a non-negotiable prohibition. Composite or aggregated routes that have no upstream equivalent are forbidden. Deviations from upstream path structure are not permitted even for cosmetic reasons.
...
-   **Source Fidelity**: FERNS API route paths after the source identifier prefix MUST be verbatim copies of the upstream source's own paths — no renaming, no compositing, no omission. Field values and response structure are preserved unchanged.
