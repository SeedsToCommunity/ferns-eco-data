...
-   **Audit tool source coverage (current)**: GBIF (`comparators/gbif.ts`), iNaturalist (`comparators/inat.ts`), BONAP (`comparators/bonap.ts`), Michigan Flora (`comparators/miflora.ts` — compares FERNS `data.spec_text` vs upstream `spec_text?id={plant_id}`), Universal FQA (`comparators/universal-fqa.ts` — checks DB list count + per-species field comparison vs upstream database blob). Static source health checks (`checks/static-sources.ts`): Coefficient of Conservatism, Wetland Indicator Status, WUCOLS, Seeds to Community Washtenaw (endpoint reachability + envelope structure + data presence). API docs and registry: `checks/docs.ts`.
