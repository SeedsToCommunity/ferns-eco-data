...
|---|-----------|------|------|--------|------------|--------------|-----------|
| 0 | `ferns-registry` | FERNS Source Registry | System | ✅ Complete | N/A (internal) | Source discovery index at `/api/v1/sources` | N/A |
...
| `seed.ts` | Populates the `ferns_sources` table for this source on first deploy |
...
- `/v1/sources` — list of all registered FERNS knowledge services
- `/v1/sources/metadata` — full metadata for the FERNS registry itself
...
### `artifacts/registry-explorer` — FERNS Data Layer (Registry Explorer)

React + Vite single-page application at `/`. Entry point for FERNS — lists all registered knowledge services grouped by `knowledge_type`.

- **Pure API consumer** — calls `GET /api/v1/sources` via `useGetSourcesIndex()`
- Groups: Source Wrappers → Derived Synthesis → Aggregations → System Components
- Cards: name, `knowledge_type` badge, status badge (only when not `live`), description, output_summary, update_frequency, known_limitations, dependency chips, Explorer + API Metadata links
- Loading, error, and empty states
- CC BY 4.0 / FERNS / Seeds to Community footer
...
### `ferns_sources` — FERNS knowledge service registry

| Column | Type | Notes |
|--------|------|-------|
| `id` | serial PK | |
| `source_id` | text UNIQUE | e.g. `bonap-napa`, `gbif`, `ferns-registry` |
| `name` | text | Display name |
| `knowledge_type` | text | `source_wrapper` \| `derived_synthesis` \| `aggregation` \| `system` |
| `status` | text | `live` \| `partial` \| `planned` |
| `description` | text | Prose description |
| `input_summary` | text | What parameters this service accepts |
| `output_summary` | text | What this service returns |
| `dependencies` | text[] | source_ids of upstream sources |
| `update_frequency` | text | Human-readable update cadence |
| `known_limitations` | text | Known caveats or gaps |
| `metadata_url` | text | URL to this source's full metadata endpoint |
| `explorer_url` | text | URL to this source's dedicated Explorer UI |
...
- `src/schema/registry.ts` — `ferns_sources` table
