...
-   **Database Schema**: Dedicated Drizzle ORM schemas exist for each source's cache tables and a central `ferns_sources` table for the registry. **Known Drizzle-kit quirk**: Named composite unique constraints defined with `unique("name").on(...)` are NOT recognized by `drizzle-kit push` 0.31.x introspection — the tool will repeatedly ask to add them even when they already exist in the DB. Workaround: use `uniqueIndex("name").on(...)` instead, which Drizzle-kit introspects correctly. This is applied to `mnfiCountyElementsTable` in `lib/db/src/schema/mnfi.ts`. Do not change this back to `unique()`.
