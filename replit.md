...
-   **USDA PLANTS (api_fetch)**: Full REST API integration using the reverse-engineered USDA NRCS PLANTS API at `https://plantsservices.sc.egov.usda.gov/api/`. Source ID: `usda-plants`. Endpoints: `GET /api/usda-plants?species={name}` (resolve scientific name → symbol → full profile), `GET /api/usda-plants/profile?symbol={sym}` (by known symbol), `GET /api/usda-plants/search?q={text}&field={field}&page={n}` (text search), `GET /api/usda-plants/metadata`. Upstream API calls: (1) `POST /PlantSearch` with `{searchText, startIndex, maxItems}` — returns `[{Text, Plant}]` array; (2) `GET /PlantProfile/{symbol}` — returns full plant object. Name-matching: `PlantSearch` entries have HTML in `ScientificName`; match by stripping HTML with regex and checking case-insensitive prefix against the queried name. Response fields: symbol, canonical_name, common_name, rank, profile_url (absolute `https://plants.sc.egov.usda.gov/?symbol={sym}`), native_statuses (array of `{Region, Status, Type}` with regions L48/AK/HI/PR/VI/CAN/etc.), wetland_data, legal_statuses, durations, growth_habits, ancestors (taxonomy chain), synonyms, fact_sheet_urls, plant_guide_urls. DB tables: `usda_plants_name_matches` (resolution cache, 30-day TTL, unique on scientific_name), `usda_plants_profiles` (profile cache, 30-day TTL, unique on symbol). Migration: `lib/db/drizzle/0003_usda_plants.sql`. Journal idx 3. Cache service: `artifacts/api-server/src/services/usda-plants/cache.ts`. Connector: `artifacts/api-server/src/services/usda-plants/connector.ts`. MCP tools: `usda_plants__species`, `usda_plants__profile`, `usda_plants__search`. Explorer page: `artifacts/registry-explorer/src/pages/UsdaPlantsPage.tsx` (Species Lookup + Search tabs). Audit comparator: `lib/ferns-audit/src/comparators/usda-plants.ts`.
