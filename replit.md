...
-   **Post-task summary (required after any significant task)**: After completing a task that involved substantial implementation or decisions, write a plain-language summary covering all of the following — do not omit sections. **This summary must be written as the body of the final git commit message** so it is preserved in the project history and visible to the main agent after merge. Write it there first, then also post it in the chat.
